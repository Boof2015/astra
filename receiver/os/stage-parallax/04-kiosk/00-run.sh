...
# TV-remote (CEC RC passthrough) keymap — see the comment in the file itself. ir-keytable merges
# EVERY rc_maps.cfg row matching a device, so appending beside Debian's stock rc-cec row leaves
# its KEY_POWER mapping live. Replace all active rc-cec rows with exactly one Parallax row; the
# stock udev hook then clears the kernel table and loads only our safe map at device creation.
# Harmless when the kernel lacks CEC_RC: no rc device ever appears.
install -D -m 0644 files/parallax_cec.toml \
  "${ROOTFS_DIR}/etc/rc_keymaps/parallax_cec.toml"
RC_MAPS_FILE="${ROOTFS_DIR}/etc/rc_maps.cfg"
RC_MAPS_TMP="$(mktemp)"
trap 'rm -f "${RC_MAPS_TMP}"' EXIT
bash files/rewrite-rc-cec-map.sh "${RC_MAPS_FILE}" > "${RC_MAPS_TMP}"
install -m 0644 "${RC_MAPS_TMP}" "${RC_MAPS_FILE}"
rm -f "${RC_MAPS_TMP}"
trap - EXIT

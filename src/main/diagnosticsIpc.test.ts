...
test('normalizes only supported PCM output routes', () => {
  ...
  assert.equal(normalizeLocalPcmOutputSink('preload_native'), 'preload_native')
...
test('legacy temporary-file changes do not disable a selected worker route', () => {
  ...
  assert.equal(resolveLegacyLocalPcmTempFileSinkChange('preload_native', true), 'temporary_file')
  ...
  assert.equal(resolveLegacyLocalPcmTempFileSinkChange('preload_native', false), 'preload_native')

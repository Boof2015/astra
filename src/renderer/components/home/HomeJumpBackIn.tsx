    ...
    <HomeSection id="jump-back-in" title="Jump back in">
      {loading ? <HomeLoadingCollectionGrid /> : (
        <HomeExpandableGrid items={cards} getKey={(card) => card.key} renderItem={(card) => (
          <HomeCollectionCard hoverPlayback title={card.title} subtitle={card.subtitle}
            revealTitle={card.source.type === 'album' || card.source.type === 'artist'}
            revealSubtitle={card.source.type === 'album' || card.source.type === 'track' ? card.subtitle : undefined}
            subtitleDetail={card.detail}

...
export default function HomeJumpBackIn({ cards, getPlayback, onOpen, onPlay }: {
  ...
  return (
    <HomeSection id="jump-back-in" title="Jump back in">
      <HomeExpandableGrid items={cards} getKey={(card) => card.key} renderItem={(card) => (
        <HomeCollectionCard title={card.title} subtitle={<>{card.subtitle} · <span>{card.detail}</span></>}

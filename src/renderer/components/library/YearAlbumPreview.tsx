...
interface YearAlbumPreviewProps {
  ...
  onPlayAlbum: (album: YearAlbumPreviewRecord) => void
        ...
        {albums.length > 0 ? (
          visibleAlbums.map((album, index) => (
            <LibraryCollectionCard
              ...
              className={`year-album-preview-card ${isExpanded && index >= layout.collapsedItemCount ? 'is-expansion-reveal' : ''}`}
              ...
              badge={album.is_new && <span className="library-latest-sync-pill year-album-preview-sync-pill" title="Added in latest library sync">NEW</span>}
            />
          ))
        ) : (

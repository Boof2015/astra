...
void Spectrum::setFFTSize(size_t size) {
    if (size != fftSize_) {
        ...
        smoothedMagnitudes_.resize(size / 2, 0.0f);
    }
}
...
const std::vector<float>& Spectrum::process(const float* audioData, size_t length) {
    ...
    for (size_t i = 0; i < magnitudes_.size(); i++) {
        ...
        float db = 20.0f * log10f(std::max(magnitudes_[i], 1e-10f));

        // Clamp to reasonable range
        db = std::clamp(db, -100.0f, 0.0f);

        // Normalize to 0-1 range (where -100dB = 0 and 0dB = 1)
        float normalized = (db + 100.0f) / 100.0f;

        // Apply smoothing
        smoothedMagnitudes_[i] = smoothing_ * smoothedMagnitudes_[i] + (1.0f - smoothing_) * normalized;
    }
